...
class TrayIcon(QSystemTrayIcon):
    def __init__(
        ...
        self.setToolTip("ActiveSeconds" + (" (testing)" if testing else ""))
    ...
    def _build_rootmenu(self) -> None:
        ...
        # This check is an attempted solution to:
        ...
        if exitIcon.availableSizes():
            menu.addAction(exitIcon, "Quit ActiveSeconds", lambda: exit(self.manager))
        else:
            menu.addAction("Quit ActiveSeconds", lambda: exit(self.manager))
...
def run(manager: Manager, testing: bool = False) -> Any:
    ...
    if not QSystemTrayIcon.isSystemTrayAvailable():
        QMessageBox.critical(
            None,
            "Systray",
            "I couldn't detect any system tray on this system. Either get one or run the ActiveSeconds modules from the console.",
        )
        sys.exit(1)
