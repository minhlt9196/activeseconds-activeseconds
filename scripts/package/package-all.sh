...
function build_zip() {
    ...
    pushd dist;
    filename="activeseconds-${version}-${platform}-${arch}.zip"
    echo "Name of package will be: $filename"

    if [[ $platform == "windows"* ]]; then
        7z a $filename activeseconds;
    else
        zip -r $filename activeseconds;
    fi
    popd;
    echo "Zip built!"
}
...
function build_setup() {
    filename="activeseconds-${version}-${platform}-${arch}-setup.exe"
    echo "Name of package will be: $filename"
...
#    env AW_VERSION=$version "$innosetupdir/iscc.exe" scripts/package/activitywatch-setup.iss
    echo $version
    env AW_VERSION=$version "iscc.exe" scripts/package/activeseconds-setup.iss
    mv dist/activeseconds-setup.exe dist/$filename
    echo "Setup built!"
}
