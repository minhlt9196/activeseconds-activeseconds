...
function build_zip() {
    ...
    pushd dist;
    filename="timebench-${version}-${platform}-${arch}.zip"
    echo "Name of package will be: $filename"

    if [[ $platform == "windows"* ]]; then
        7z a $filename timebench;
    else
        zip -r $filename timebench;
    fi
    popd;
    echo "Zip built!"
}
...
function build_setup() {
    filename="timebench-${version}-${platform}-${arch}-setup.exe"
    echo "Name of package will be: $filename"
...
#    env AW_VERSION=$version "$innosetupdir/iscc.exe" scripts/package/activitywatch-setup.iss
    echo $version
    env AW_VERSION=$version "iscc.exe" scripts/package/timebench-setup.iss
    mv dist/timebench-setup.exe dist/$filename
    echo "Setup built!"
}
