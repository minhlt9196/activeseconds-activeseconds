...
package:
	mkdir -p dist/timebench
#
	make --directory=aw-watcher-afk package
	cp -r aw-watcher-afk/dist/aw-watcher-afk dist/timebench
#
	make --directory=aw-watcher-window package
	cp -r aw-watcher-window/dist/aw-watcher-window dist/timebench
#
	make --directory=aw-server package
	cp -r aw-server/dist/aw-server dist/timebench
#
	make --directory=aw-server-rust package
	mkdir -p dist/timebench/aw-server-rust
	cp -r aw-server-rust/target/package/* dist/timebench/aw-server-rust
#
	make --directory=aw-qt package
	cp -r aw-qt/dist/aw-qt/. dist/timebench
# Remove problem-causing binaries, see https://github.com/ActivityWatch/activitywatch/issues/161 activitywatch
	rm -f dist/timebench/libdrm.so.2
# Remove unecessary files
	rm -rf dist/timebench/pytz
