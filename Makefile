...
package:
	mkdir -p dist/activeseconds
#
	make --directory=aw-watcher-afk package
	cp -r aw-watcher-afk/dist/aw-watcher-afk dist/activeseconds
#
	make --directory=aw-watcher-window package
	cp -r aw-watcher-window/dist/aw-watcher-window dist/activeseconds
#
	make --directory=aw-server package
	cp -r aw-server/dist/aw-server dist/activeseconds
#
	make --directory=aw-server-rust package
	mkdir -p dist/activeseconds/aw-server-rust
	cp -r aw-server-rust/target/package/* dist/activeseconds/aw-server-rust
#
	make --directory=aw-qt package
	cp -r aw-qt/dist/aw-qt/. dist/activeseconds
# Remove problem-causing binaries, see https://github.com/ActivityWatch/activitywatch/issues/161 activitywatch
	rm -f dist/activeseconds/libdrm.so.2
# Remove unecessary files
	rm -rf dist/activeseconds/pytz
